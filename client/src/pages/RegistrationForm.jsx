...
const RegistrationForm = () => {
  ...
  const validate = () => {
    ...
    if (!/^[1-9]|1[0-2]$/.test(formData.Semestre)) {
      newErrors.Semestre = "El semestre debe ser un número entre 1 y 12.";
    }

    if (!/^[0-9]{7,15}$/.test(formData.NúmeroContacto)) {
      newErrors.NúmeroContacto = "El número de contacto debe contener solo números y al menos 7 dígitos.";
    }

    if (formData.linkedinOrFacebook.trim() && !/^https?:\/\/.+/.test(formData.linkedinOrFacebook)) {
      newErrors.linkedinOrFacebook = "Debe ser una URL válida.";
    }
    ...
    <form onSubmit={handleSubmit} className="max-w-3xl mx-auto bg-gray-800 p-8 rounded-lg">
      {Object.entries(formData).map(([key, value]) => (
        <div key={key} className="mb-4">
          ...
          <input
            type={key === "email" ? "email" : key === "NúmeroContacto" ? "tel" : "text"}
            name={key}
            value={value}
            onChange={handleChange}
            className="mt-1 block w-full px-4 py-2 bg-gray-700 text-white rounded-lg focus:ring-yellow-400 focus:border-yellow-400"
      ...
      <button type="submit" className="bg-yellow-400 text-black px-8 py-3 rounded-lg text-lg font-semibold hover:bg-yellow-300 transform hover:scale-105 transition-all">
        Enviar Registro
      </button>
    </form>
  );
